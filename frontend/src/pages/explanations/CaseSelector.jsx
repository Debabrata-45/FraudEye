    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 mb-1.5">
        <div className="flex items-center gap-1.5">
          <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${RISK_DOT[xcase.fraudLabel] || 'bg-slate-500'}`} />
          <span className={`text-[11px] font-mono font-semibold ${isSelected ? 'text-cyan-300' : 'text-slate-300'}`}>
            {xcase.txnId}
          </span>
        </div>
      ...
      <div className="flex items-center justify-between">
        <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded border ${labelCfg.bg} ${labelCfg.border} ${labelCfg.text}`}>
          {xcase.fraudLabel}
        </span>
        <span className="text-[10px] text-slate-600">
          {formatDistanceToNow(xcase.timestamp, { addSuffix: true })}
        </span>
      </div>
    </motion.button>
  );
...
const CaseSelector = ({ cases, selectedId, onSelect, loading }) => (
  ...
  >
    ...
    <div className="px-1 pb-3 border-b border-slate-800 mb-3 flex-shrink-0">
      ...
      <p className="text-xs text-slate-600 mt-0.5">{cases.length} explanations available</p>
    </div>
