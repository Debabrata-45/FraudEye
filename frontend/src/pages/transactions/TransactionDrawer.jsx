...
import {
  X, Shield, AlertTriangle, MapPin,
  Smartphone, CreditCard, User, ChevronRight, Brain, TrendingUp,
} from 'lucide-react';
import {
  RISK_LEVELS,
  STATUS_CONFIG,
...
const MetaRow = ({ label, value, mono, accent }) => (
  <div className="flex items-center justify-between py-1.5 border-b border-slate-800/60 last:border-0">
    ...
    <span className={`text-xs font-medium ${mono ? 'font-mono' : ''} ${accent || 'text-slate-200'}`}>
      {value}
    </span>
  </div>
);
  ...
  return (
    <div className="mb-2 last:mb-0">
      <div className="flex items-center justify-between mb-1">
        <span className="text-[11px] text-slate-400 truncate max-w-[160px]">{feature}</span>
        <span className={`text-[11px] font-semibold tabular-nums ${positive ? 'text-rose-400' : 'text-emerald-400'}`}>
...
const ReasonTag = ({ reason, risk }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-[11px] font-medium border mr-1.5 mb-1.5
      ${isCritical
...
const DrawerEmpty = () => (
  <div className="flex flex-col items-center justify-center h-full py-16 gap-4">
    ...
    <div className="text-center">
      ...
      <p className="text-xs text-slate-600 mt-1">Click any row to investigate details</p>
    </div>
  </div>
);
...
const TransactionDrawer = ({ transaction: tx, onClose }) => {
  ...
  const fraudCfg  = tx ? (FRAUD_LABEL_CONFIG[tx.fraudLabel] || FRAUD_LABEL_CONFIG.UNKNOWN) : null;
    ...
    >
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-slate-100">Transaction Detail</span>
        </div>
        {onClose && (
          <button
            onClick={onClose}
        ...
        <AnimatePresence mode="wait">
          {!tx ? (
            ...
            >
              ...
              {/* Risk headline */}
              <div className={`p-4 rounded-xl border mb-5 ${riskCfg.bg} ${riskCfg.border}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className="w-2 h-2 rounded-full"
                        style={{ backgroundColor: riskCfg.color, boxShadow: `0 0 8px ${riskCfg.color}` }}
                      />
                      <span className={`text-sm font-bold ${riskCfg.text}`}>{riskCfg.label} Risk</span>
                    </div>
                  ...
                  <div className="text-right">
                    <div className={`text-2xl font-black tabular-nums ${riskCfg.text}`}>
                      {Math.round(tx.riskScore * 100)}%
                    </div>
                    <div className="text-[10px] text-slate-500 mt-0.5">Fraud probability</div>
                  </div>
                </div>
              ...
              <div className="flex gap-2 mb-5">
                <div className="flex-1 px-3 py-2 rounded-lg bg-slate-800/60 border border-slate-700/40">
                  <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Reference</div>
                  <div className="text-xs font-mono font-semibold text-cyan-300">{tx.id}</div>
                </div>
                <div className="flex-1 px-3 py-2 rounded-lg bg-slate-800/60 border border-slate-700/40">
                  <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Verdict</div>
                  <span className={`text-xs font-semibold ${fraudCfg.text}`}>{fraudCfg.label}</span>
                </div>
                <div className="flex-1 px-3 py-2 rounded-lg bg-slate-800/60 border border-slate-700/40">
              ...
              <DrawerSection title="Transaction Details" icon={CreditCard}>
                <div className="bg-slate-800/30 rounded-lg px-3 py-1 border border-slate-700/30">
                  <MetaRow label="Timestamp"   value={formatExactTime(tx.timestamp)} mono />
                  <MetaRow label="Merchant"    value={`${tx.merchant.icon} ${tx.merchant.name}`} />
                  <MetaRow label="Category"    value={tx.merchant.category} />
                  <MetaRow label="Card ending" value={`•••• ${tx.cardLast4}`} mono />
                  <MetaRow label="Model confidence" value={`${tx.modelConfidence}%`} accent="text-cyan-300" />
                </div>
              </DrawerSection>
              ...
              <DrawerSection title="Account Context" icon={User}>
                <div className="bg-slate-800/30 rounded-lg px-3 py-1 border border-slate-700/30">
                  ...
                  <MetaRow
                    label="Location"
                    value={<span className="flex items-center gap-1"><MapPin size={10} />{tx.geo}</span>}
                  />
                  <MetaRow
                    label="Device"
                    value={<span className="flex items-center gap-1"><Smartphone size={10} />{tx.device}</span>}
                  />
                </div>
              </DrawerSection>
              ...
              <DrawerSection title="AI Feature Importance (SHAP)" icon={Brain}>
                <div className="bg-slate-800/30 rounded-lg px-3 py-3 border border-slate-700/30">
                  ...
                  <p className="text-[10px] text-slate-600 mt-3 flex items-center gap-1">
                    <span className="w-2 h-0.5 bg-rose-500 rounded" /> Increases fraud risk
                    <span className="ml-2 w-2 h-0.5 bg-emerald-500 rounded" /> Reduces fraud risk
                  </p>
                </div>
              </DrawerSection>
              ...
              <DrawerSection title="Analyst Actions" icon={TrendingUp}>
                <div className="flex flex-col gap-2">
                  <button className="flex items-center justify-between w-full px-3 py-2.5 rounded-lg bg-rose-500/10 border border-rose-500/25 text-rose-300 hover:bg-rose-500/20 hover:border-rose-500/40 transition-all group">
                    <span className="text-xs font-semibold">Flag as Fraud</span>
                  ...
                  <button className="flex items-center justify-between w-full px-3 py-2.5 rounded-lg bg-emerald-500/10 border border-emerald-500/25 text-emerald-300 hover:bg-emerald-500/20 hover:border-emerald-500/40 transition-all group">
                    <span className="text-xs font-semibold">Clear as Legitimate</span>
                    <ChevronRight size={13} className="opacity-50 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all" />
                  </button>
                  <button className="flex items-center justify-between w-full px-3 py-2.5 rounded-lg bg-violet-500/10 border border-violet-500/25 text-violet-300 hover:bg-violet-500/20 hover:border-violet-500/40 transition-all group">
                    <span className="text-xs font-semibold">Open Full Explanation</span>
                    <ChevronRight size={13} className="opacity-50 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all" />
                  </button>
                </div>
              </DrawerSection>
